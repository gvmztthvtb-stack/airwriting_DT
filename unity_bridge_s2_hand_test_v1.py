...
버전: tp-unity-bridge-s2-hand-test-v1

손가락 S3 배선 수리 전, 손등 S2 IMU로 Unity UI/Trail/범위를 임시 테스트하는 파일입니다.
...
중요한 한계
-----------
이 테스트 코드는 S2 손등 IMU의 '방향 변화'를 2D 포인터로 변환합니다.
...
BRIDGE_VERSION = "tp-unity-bridge-s2-hand-test-v1"
...
# 임시 하드웨어 테스트용 포인터 센서 선택
# "S2_HAND" : 손등 MPU-6050으로 포인터 제어
# "S3_FINGER": 손가락 ICM-20948로 최종 포인터 제어
POINTER_SENSOR_SOURCE = "S2_HAND"
...
def selected_pointer_sensor(
    frame: SensorFrame,
) -> tuple[Vector3, Vector3, Vector3, str]:
    """
    현재 테스트 모드에서 포인터에 사용할 IMU를 선택합니다.
    S2가 없는 68/70B 패킷에서는 S1(전완)을 임시 대체값으로 사용합니다.
    """
    if POINTER_SENSOR_SOURCE == "S2_HAND":
        accel = (
            frame.hand_accel
        ...
        return accel, gyro, (0.0, 0.0, 0.0), "S2_HAND_IMU"
...
def pointer_sensor_valid(frame: SensorFrame) -> tuple[bool, str]:
    accel, gyro, _mag, _source = selected_pointer_sensor(frame)
...
def send_unity(
    ...
    *,
    ...
    message = {
        ...
        "pointer_source": POINTER_SENSOR_SOURCE,
...
def main() -> None:
    ...
    print(
        ...
        f"pointer_source={POINTER_SENSOR_SOURCE}",
    ...
    try:
        while True:
            ...
            for packet in extract_packets(receive_buffer):
                ...
                valid_sensor, invalid_reason = pointer_sensor_valid(frame)
                now = time.time()
                (
                    pointer_accel,
                    pointer_gyro,
                    pointer_mag,
                    pointer_source_label,
                ) = selected_pointer_sensor(frame)
                    ...
                    if elapsed > LONG_SENSOR_LOST_SEC:
                        ...
                        if not sensor_lost_printed:
                            print(
                                ...
                                f"source={pointer_source_label}",
                                f"|accel|={vector_norm(pointer_accel):.3f}",
                                f"|gyro|={vector_norm(pointer_gyro):.3f}",
                            )
                            print(
                ...
                if sensor_lost_printed:
                    print(f"[포인터 IMU 복구] source={pointer_source_label}")
                    sensor_lost_printed = False
                ...
                if len(finger_gyro_samples) < GYRO_CALIBRATION_SAMPLES:
                    ...
                    finger_gyro_norm = vector_norm(pointer_gyro)
                    ...
                    if (
                        ...
                        finger_gyro_samples.append(pointer_gyro)
                        hand_gyro_samples.append(hand_gyro)

                        if (
                            POINTER_SENSOR_SOURCE == "S3_FINGER"
                            and vector_norm(pointer_mag) > 1e-6
                        ):
                            mag_samples.append(pointer_mag)
                    ...
                    if now - last_calibration_print >= 0.5:
                        ...
                        print(
                            ...
                            f"{pointer_source_label}|a|={vector_norm(pointer_accel):.3f} "
                            f"{pointer_source_label}|g|={finger_gyro_norm:.3f}"
                        )
                ...
                current_horizontal, current_vertical = (
                    ray_projector.project(pointer_q)
                )
                ...
                if (
                    ...
                    if (
                        ...
                        if POINTER_SENSOR_SOURCE == "S3_FINGER":
                            mag_yaw.calibrate(
                                mag_samples,
                                finger_q,
                            )
